  ...
  const handleFileSelect = (index: number, file: File | null) => {
   ...
   const saveToBackend = async () => {
    try {
      ...
      console.log("[HeroAdmin] PUT /admin/banner - saving images", images);
      const res = await api.put("/admin/banner", { images });
      console.log("[HeroAdmin] PUT /admin/banner response", res?.status, res?.data);
      toast({ title: "Banner saved", description: `${images.length} image(s)` });
    } catch (error: any) {
      console.error("[HeroAdmin] PUT /admin/banner error", error?.message, error?.response?.data);
