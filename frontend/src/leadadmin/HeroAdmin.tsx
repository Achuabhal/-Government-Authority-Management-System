...
const HeroAdmin = () => {
  ...
  useEffect(() => {
    const fetchSlides = async () => {
      try {
        setLoading(true);
        const response = await api.get("/leadadmin/banner");
  ...
  const handleFileSelect = (index: number, file: File | null) => {
   ...
   const saveToBackend = async () => {
    try {
      ...
      console.log("[HeroAdmin] PUT /leadadmin/banner - saving images", images);
      const res = await api.put("/leadadmin/banner", { images });
      console.log("[HeroAdmin] PUT /leadadmin/banner response", res?.status, res?.data);
      toast({ title: "Banner saved", description: `${images.length} image(s)` });
    } catch (error: any) {
      console.error("[HeroAdmin] PUT /leadadmin/banner error", error?.message, error?.response?.data);
